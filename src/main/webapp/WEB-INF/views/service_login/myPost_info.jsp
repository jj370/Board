							...
							<h2>작성한 글 관리</h2>
								...
								<table style="font-size:0.9em; text-align: center;width: 750px; margin: 0 auto; height : 60px; background-color: white;">
									<tr>
										...
										<c:choose>
											<c:when test="${info_list.size() eq 0}">
												<tr>
											...
											<c:forEach items="${info_list }" var="list">
												...
												<c:choose>
													<c:when test="${savedate==today}">
														<td style="text-align:left;"><a href="info_post?num=${list.num }">[${list.tag }]${list.title }</a>
												...
												<c:otherwise>
													<td style="text-align:left;"><a href="info_post?num=${list.num }">[${list.tag }]${list.title }</a></td>
													<fmt:formatDate var="savedate" value="${list.savedate }" pattern="yyyy-MM-dd"/>
													<td style="text-align:ceter;">${savedate }</td>
